...
-- 각 부서별(NULL인 경우는 부서없음)
-- 사원의 수, 보너스를 받는 사원의 수, 총 급여, 평균 급여, 최저 급여, 최고 급여 조회
-- (부서 별 오름차순 정렬, 부서 없음이 맨 위에)
SELECT 
    NVL(DEPT_CODE, '부서없음'),
    COUNT(*), COUNT(BONUS), SUM(SALARY), 
    FLOOR(AVG(NVL(SALARY,0))), MIN(SALARY), MAX(SALARY)
FROM EMPLOYEE 
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE NULLS FIRST;   -- 1로 쓰면 NULLS FIRST 작동 X 
...
SELECT DEPT_CODE, TO_CHAR(FLOOR(AVG(NVL(SALARY,0))), 'FM999,999,999') "평균 급여"
FROM EMPLOYEE
GROUP BY DEPT_CODE  -- 여기까지가 부서별 평균급여
HAVING FLOOR(AVG(NVL(SALARY,0))) >= 3000000
ORDER BY DEPT_CODE;

-- 직급별 총 급여의 합이 1000만원 이상인 직급만 조회
SELECT JOB_CODE, TO_CHAR(SUM(SALARY), 'FM999,999,999') "총 급여의 합"
FROM EMPLOYEE
GROUP BY JOB_CODE
...
/*
    ...
    # UNION(합집합) : 두 쿼리문을 수행한 결과값을 하나로 합쳐서 추출 ( 중복되는 행은 제거됨)
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE SALARY > 3000000;  -- 중복이 제거되고 12개 나옴

-- 위 쿼리문 대신 WHERE 절에 OR연산자를 사용해서 처리 가능
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
...
FROM EMPLOYEE
WHERE SALARY > 3000000
ORDER BY EMP_NAME; -- 중복이 제거되고지 않고 14개 나옴


-- 3. INTERSECT
-- 부서코드가 'D5'이면서 급여가 300만원 초과인 사원 조회하는 것
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
INTERSECT
...
-- 4. MINUS
-- 부서코드가 'D5'인 사원들 중에서 급여가 300만원 초과인 사원들을 제외해서 조회하는 것
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
MINUS
