...
-- 일반타입변수 TEAM을 데이터 타입 VARCHAR2(10)으로 선언하고 NCODE 값이 KO일 경우 => TEAM에 '국내팀' 대입,
...
-- 사용자에게 입력받은 사번과 일치하는 사원의 급여 조회 후 출력 (조회한 급여는 SAL변수에 대입)
...
-- DEPT_CODE에 따라서 알맞는 부서를 출력(DNAME, VARCHAR2(30))
-- D1인 경우 인사관리부, D2인 경우 회계관리부, D3인 경우 마케팅부, D4인 경우 국내영업부, D5인 경우 해외영업1부, D6인 경우 해외영업2부, D7인 경우 해외영업3부
...
EXCEPTION
    WHEN ZERO_DIVIDE THEN DBMS_OUTPUT.PUT_LINE('나누기 연산시 0 으로 나눌 수 없습니다.');
END;
/
...
-- NO_DATA_FOUND: NO_DATA_FOUND : SELECT한 결과가 한 행도 없을 경우
-- TOO_MANY_ROWS : SELECT한 결과가 여러 행일 경우(한 행이 리턴되어야 하는데..)
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
