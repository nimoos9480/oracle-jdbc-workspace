...
/*
    ...
    - 두 테이블 간의 JOIN 시 일치하지 않는 행도 포함시켜서 조회가 가능함
    - 단, 반드시 기준이 되는 테이블(컬럼)을 지정해야 한다.
        (LEFT, RIGHT, FULL, (+))
*/
-- 사원명, 부서명, 급여, 연봉 조회(내부조인 방식)
-- 부서배치가 아직 안 된 사원 2명에 대한 정보는 조회 X
SELECT EMP_NO, DEPT_TITLE, SALARY, SALARY*12 AS 연봉
FROM EMPLOYEE, DEPARTMENT
WHERE  DEPT_CODE = DEPT_ID;
...
--1) LEFT [OUTER] JOIN : 두 테이블 중 왼편에 기술되 테이블을 기준으로 JOIN
...
SELECT EMP_NO, DEPT_TITLE, SALARY, SALARY*12 AS 연봉
FROM EMPLOYEE, DEPARTMENT
WHERE  DEPT_CODE(+) = DEPT_ID(+); -- (+)는 무조건 1번만 사용 가능
...
FROM EMPLOYEE E
JOIN EMPLOYEE M ON (E.EMP_ID=M.EMP_ID);
...
FROM EMPLOYEE
CROSS JOIN DEPARTMENT;  -- 직원 23명 X 부서 9개
...
SELECT EMP_NAME 직원명, BONUS, SALARY*12 연봉, NVL(DEPT_TITLE, '부서없음') 부서명, LOCAL_NAME 근무지역
FROM EMPLOYEE E
...
-- 5. 각 부서별 평균 급여를 조회하여 부서명, 평균 급여(정수 처리)를 조회                              --다시!!
--     단, 부서 배치가 안된 사원들의 평균도 같이 나오게끔!
SELECT NVL(DEPT_TITLE, '부서 없음') , TO_CHAR(ROUND(AVG(SALARY),0), 'FM999,999,999') "평균 급여"
FROM EMPLOYEE E
LEFT JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
GROUP BY DEPT_TITLE
ORDER BY DEPT_TITLE;
