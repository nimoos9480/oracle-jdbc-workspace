...
INSERT INTO EMP_01
VALUES(200, '이병헌', '개발 지원팀'); -- 값의 수가 충분하지 않음

-- 에러는 발생하지 않지만 데이터가 잘못 저장
INSERT INTO EMP_01
VALUES(300, '유아지원팀', '공유', DEFAULT); -- INSERT 됨 => 값의 순서 주의

-- 컬럼 순서와 데이터 타입이 맞지 않아서 에러 발생
INSERT INTO EMP_01
VALUES('유아지원팀', 300, '위하준', DEFAULT);  -- 수치가 부적합
...
INSERT INTO EMP_01 (
    SELECT EMP_ID, EMP_NAME, DEPT_TITLE, HIRE_DATE   -- 컬럼 개수나 순서 주의하기
    FROM EMPLOYEE
    JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
);    
...
-- EMP_NAMAGER 테이블에 EMPLOYEE 테이블의 부서 코드가 D1인 직원의 사번, 이름, 관리자 사번을 조회하여 삽입한다.
...
SELECT EMP_ID  -- 아시아지역에서 근무하는 직원들
FROM EMP_SALARY
...
/*
    ...
    - 별도의 조건을 제시할 수 없고 ROLLBACK이 불가능함!!   -- 데이터만 지우는 것, 테이블은 남아있음  ==> 테이블 지울 때는 DROP!
