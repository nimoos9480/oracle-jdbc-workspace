...
SELECT EMP_ID, EMP_NAME, JOB_CODE, TO_CHAR(SALARY, 'FM999,999,999')||'원' "급여"
FROM EMPLOYEE
...
/*
    ...
    - 일반 비교연산자 사용 가능 : =, !=, ^=, > , <, >=, ...
*/
-- 노옹철 사원의 급여보다 더 많이 받은 사원의 사번, 사원명, 부서명, 직급코드, 급여 조회
SELECT SALARY
FROM EMPLOYEE
WHERE EMP_NAME = '노옹철';  --370000
...
FROM EMPLOYEE
ORDER BY SALARY DESC; -->> ROWNUM이 엉망인 이유 : ORDER BY 들어가기 전 순서이기 때문
...
SELECT ROWNUM, DEPT_CODE , 평균급여                                             -- 별칭을 사용해준다!
...
-- 상위 5명만 조회
-- RANK 함수는 WHERE절에서 사용할 수 없다! (ORDER BY가 SELECT에서 실행되고 내려오기 때문에 가능)
SELECT 
    RANK() OVER(ORDER BY SALARY DESC),
    EMP_NAME, 
    SALARY
