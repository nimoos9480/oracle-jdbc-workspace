/*
   ...
   1. DDL(Data Definition Language) : 데이터 정의어
     - DBMS의 구조를 정의하거나 변경, 삭제하기 위해 사용하는 언어
      (CREATE 생성, ALTER 수정, DROP 삭제) - 테이블을 다룸
      
   2. DML(Data Manipulation Language) : 데이터 조작어 ★
     - 실제 데이터를 조작하기 위해 사용하는 언어
      (INSERT 삽입, UPDATE 수정, DELETE 삭제) - 데이터를 다룸
...
-- EMPLOYEE 테이블에서 직원명(EMP_NAME), 직원의 연봉(= 급여 SALARY * 12) 조회 ( => 산술 연산을 함께 할 수 있음)
...
-- EMPLOYEE에 사원명, 입사일, 근무일수(오늘날짜 - 입사일)  ( => 날짜도 연산이 가능)
-- 오늘날짜 : SYSDATE
...
/*
  리터럴 (문자열)
  - SELECT 절에 리터럴을 사용하면 테이블에 존재하는 데이터처럼 조회가 가능함
  - 즉, 리터럴을 RESULT SET의 모든 행에 반복적으로 출력된다.
*/
...
-- ★ 유의사항! DISTINCT는 SELECT 절에 딱 한 번만 기술 가능
-- EMPLOYEE의 직급코드, 부서코드 조회
...
/*
  ...
  - 조회하고자 하는 테이블로부터 "특정 조건"에 만족하는 데이터만을 조회하고자 할 때 사용
  - 이때 WHERE 절에 조건식을 제시
...
SELECT EMP_NAME 사원명, SALARY || '원' 급여, DEPT_CODE 부서코드
FROM EMPLOYEE
-- WHERE DEPT_CODE != 'D1';
-- WHERE DEPT_CODE ^= 'D1';
WHERE DEPT_CODE <> 'D1';
...
FROM EMPLOYEE
WHERE SALARY >= 4000000;   -- 숫자는 ''안써도 인식O

-- 재직중(ENT-YN 컬럼값이 'N')인 사원들의 사번, 사원명, 입사일 조회
...
-- < OR(또는), AND(그리고) >
-- 1. 부서코드가 'D6'이거나 급여가 300만원 이상이 사원들의 사원명, 부서코드, 급여 조회
SELECT EMP_NAME 사원명, DEPT_CODE 부서코드, SALARY || '원' 급여 
FROM EMPLOYEE
...
/*
  ...
  비교대상컬럼 LIKE '%문자%' => 비교대상컬럼값에 문자가 "포함"되는 걸 조회(키워드 검색 시)
  
  '_' : 1글자를 뜻함
  비교대상컬럼 LIKE '_문자' => 비교대상컬럼값에 문자앞에 무조건 한글자가 올 결우 조회
  비교대상컬럼 LIKE '__문자' => 비교대상컬럼값에 문자앞에 무조건 두글자가 올 경우 조회(언더바 2번)
  비교대상컬럼 LIKE '_문자_' => 비교대상컬럼값에 문자 앞과 뒤에 무조건 한 글자씩 올 경우 조회
...
SELECT EMP_ID 사번, EMP_NAME 사원명, SALARY ||'원' 급여, (SALARY + (BONUS * SALARY)) * 12 AS 보너스포함연봉
FROM EMPLOYEE
...
/*
  ORDER BY 절
  - SELECT문 가장 마지막 줄에 작성할 뿐만 아니라 실행순서 또한 마지막에 실행
  ...
  - NULLS FIRST : 정렬하고자하는컬럼값에 NULL이 있을 경우 해당 데이터를 맨 앞에 배치 (생략시 DESC일때 기본값)
  - NULLS LAST : 정렬하고자하는컬럼값에 NULL이 있을 경우 해당 데이터를 맨 뒤에 배치 (생략시 ASC일때 기본값)
*/

-- 전 사원의 사원명, 보너스 조회 (보너스 기준 오름차순 정렬)
SELECT EMP_NAME, BONUS
FROM EMPLOYEE
-- WHERE BONUS IS NOT NULL -- NULL값 없애고 싶다면
-- ORDER BY BONUS; -- 보너스 기준 오름차순 정렬(NULL이 맨뒷쪽 : 기본값)
-- ORDER BY BONUS DESC; -- 보너스 기준 내림차순 정렬(NULL이 앞쪽 : 기본값)
